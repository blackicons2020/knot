...
const OnboardingFlow: React.FC<OnboardingFlowProps> = ({ user, onComplete, onCancel }) => {
  ...
  const handleNext = () => {
    ...
    if (step === 5 && !formData.isVerified) {
        alert("Verification is mandatory to complete your signup. Please verify your identity.");
        return;
    }
    if (step < totalSteps) {
      setStep(step + 1);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (result) setFormData({ ...formData, profileImageUrls: [result] });
        ...
        {step === 3 && (
          <div className="space-y-8">
            ...
            <div className="space-y-5">
              ...
              <div>
                <label className={labelClass}>Preferred Partner Age Range</label>
                <div className="flex items-center gap-4">
                  <div className="flex-1">
                    <label className="text-[9px] text-gray-400 uppercase font-bold">Min Age</label>
                    <input 
                      type="number" 
                      value={formData.preferredPartnerAgeRange?.[0] || 18} 
                      onChange={(e) => {
                          const val = parseInt(e.target.value) || 18;
                          setFormData(prev => ({ 
                              ...prev, 
                              preferredPartnerAgeRange: [val, prev.preferredPartnerAgeRange?.[1] || 99] 
                          }));
                      }}
                      className={inputClass}
                    />
                  ...
                  <div className="flex-1">
                    <label className="text-[9px] text-gray-400 uppercase font-bold">Max Age</label>
                    <input 
                      type="number" 
                      value={formData.preferredPartnerAgeRange?.[1] || 99} 
                      onChange={(e) => {
                          const val = parseInt(e.target.value) || 99;
                          setFormData(prev => ({ 
                              ...prev, 
                              preferredPartnerAgeRange: [prev.preferredPartnerAgeRange?.[0] || 18, val] 
                          }));
                      }}
                      className={inputClass}
                    />
              ...
              <div>
                <label className={labelClass}>Marriage Expectations</label>
                <textarea 
                  name="marriageExpectations" 
                  rows={3} 
        ...
        {step === 5 && (
          <div className="space-y-8 text-center px-4">
            ...
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">
              Verified users get 3x more matches and priority display
            </p>
          </div>
        )}
      </div>
