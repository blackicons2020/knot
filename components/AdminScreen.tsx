...
const AdminScreen: React.FC<AdminScreenProps> = ({ onBack }) => {
    ...
    const handleSeedData = async () => {
        setIsLoading(true);
        try {
            const { MATCHES_DATA } = await import('../constants');
    ...
    const approveVerification = async (memberId: string) => {
        try {
            ...
            if (member) {
                const updatedMember = { ...member, isVerified: true };
                await db.saveUser(updatedMember);
    ...
    const revokeVerification = async (memberId: string) => {
        if (window.confirm("Revoke this member's verified status?")) {
            try {
                const member = members.find(m => m.id === memberId);
                if (member) {
                    const updatedMember = { ...member, isVerified: false };
                    await db.saveUser(updatedMember);
                    setMembers(prev => prev.map(m => m.id === memberId ? updatedMember : m));
            ...
            <header className="flex items-center justify-between p-4 bg-brand-dark text-white border-b border-white/10 sticky top-0 z-10 shadow-xl">
                ...
                <div className="flex items-center gap-3">
                    <button 
                        ...
                        disabled={isLoading}
                        className="bg-brand-accent text-brand-dark px-4 py-2 rounded-xl font-black uppercase text-[10px] tracking-widest hover:opacity-80 transition-opacity disabled:opacity-50"
                    >
                        {isLoading ? 'SEEDING...' : 'SEED MOCK DATA'}
                    </button>
                            ...
                            {filteredMembers.map((member) => (
                                <div key={member.id} className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm flex flex-col gap-4">
                                    <div className="flex items-center gap-4">
                                        <img src={member.profileImageUrls?.[0] || 'https://picsum.photos/seed/user/200'} alt={member.name} className="w-16 h-16 rounded-full object-cover shadow-inner border-2 border-white" />
