...
const LikesScreen: React.FC<LikesScreenProps> = ({ likedMatches, onMatchClick, user, onUpgrade }) => {
  return (
    <div className="p-4 pb-24 bg-white dark:bg-brand-dark min-h-screen transition-colors">
      <h1 className="text-2xl font-black text-brand-dark dark:text-white mb-4 uppercase tracking-tight">Who Likes You</h1>
      {!user.isPremium ? (
        ...
        <div className="text-center py-20">
            <p className="text-gray-600 dark:text-gray-400 font-bold">No one has liked your profile yet.</p>
            <p className="text-sm text-gray-500 dark:text-gray-500 mt-2">Keep exploring and check back soon!</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 gap-4">
          {likedMatches.map((match) => (
            <div key={match.id} onClick={() => onMatchClick(match)} className="cursor-pointer group relative rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-shadow border border-gray-100 dark:border-gray-800">
              ...
              <div className="absolute bottom-0 left-0 p-3 text-white">
                <p className="font-black text-xs uppercase tracking-tight truncate">{match.name}, {match.age}</p>
              </div>
            </div>
          ))}
        </div>
