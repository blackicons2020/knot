...
const DiscoveryScreen: React.FC<DiscoveryScreenProps> = ({ matches, user, onMatchClick, onOpenFilters }) => {
  const processedMatches = useMemo(() => {
    ...
    // Priority: Higher subscriptionAmount first, then by localScore
      ...
      <div className="p-4 pt-4">
        ...
        {processedMatches.length === 0 && (
          <div className="flex flex-col items-center justify-center py-32 text-center px-8 bg-white dark:bg-gray-900 rounded-[2.5rem] mt-4 border border-dashed border-gray-200 dark:border-gray-800">
            <p className="text-gray-400 dark:text-gray-500 font-bold uppercase tracking-widest text-xs">No users found in the registry.</p>
            <button onClick={onOpenFilters} className="mt-4 text-brand-primary dark:text-brand-accent font-black uppercase text-[10px] tracking-widest underline">Adjust Filters</button>
          </div>
        )}
      </div>
    </div>
